...
Сервис с возможностью создавать динамические API через диалог с нейросетью
...
```json
{
  "message": "Generate api config to get all fields except password_hash from users from previously created table using email as query parameter. Don't include parameter into path, only into parameter list.",
  "chatId": "<chatId>"
}
```
...
12. Вызвать новый эндпоинт еще раз, передав в качестве параметра запроса один из email ранее добавленных нами пользователей.
    Вернутся поля этого пользователя.